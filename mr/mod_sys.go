...
type System struct {
	...
	// TrafficStats: monthly traffic per device and WAN device (mon module, mod_mon_traffic.go)
	TrafficStats bool `yaml:"traffic_stats,omitempty"`
	// History: config snapshots / revisions kept (default 20)
	History int `yaml:"history,omitempty"`
	// Watchcat: escalating recovery when every WAN is down (mod_sys_watchcat.go)
	Watchcat Watchcat `yaml:"watchcat,omitempty"`
}
...
func init() {
	register(&Module{
		...
		Verify: func(c *Config, restarted []string) []string {
			...
			os.Remove(upgradePlanFile) // a new firmware's plan check is answered by this apply
			...
			updateLEDs(c)              // leds-off / wifi-off windows as the (new) schedules have them
			wifiWindow(c)
			return edgeVerify(c, restarted)
		},
		Secrets: func(c *Config) []string {
			s := append(ddnsSecrets(c), notifySecrets(c)...)
			for _, t := range []string{c.Services.Edge.ACME.Token, c.Services.Edge.ACME.Key} {
				if t != "" {
					s = append(s, t)
				}
			}
			for _, r := range c.Services.Edge.Routes {
				if r.Auth.Password != "" {
					s = append(s, r.Auth.Password)
				}
			}
			return s
		},
		TokenScope: map[string]string{"sys.speedtest": "operate", "sys.services": "read", "sys.time": "read", "sys.doctor": "read", "sys.events": "read", "sys.edge": "read", "sys.heal": "operate", "service": "operate", "diag": "operate"},
