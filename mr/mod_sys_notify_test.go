...
func TestNotifyValidate(t *testing.T) {
	...
	// defaults: rate, every event but apply, a background doctor while channels exist, json webhooks
	c.Notify = Notify{Channels: []NotifyChannel{{Name: "hook", Type: "webhook", URL: "hook_ok"}}}
	c.defaults()
	mustValid(t, c)
	n := c.Notify
	if n.Rate != 10 || strings.Join(n.Events, ",") != "wan_down,wan_up,failover,rollback,login_lock,new_device,boot,upgrade,doctor" ||
